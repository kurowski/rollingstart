#!/usr/bin/env bash
# Create one GitHub issue per sub-scope in a checkpoint plan, assign the
...
#
# Example:
#   create-issues.sh docs/plans/p1a-write-mode.md 1a 3
#
# Expectations about the plan file:
#   - Has a "## Sub-scopes" H2 section
#   - Each sub-scope begins "### 1a.M — Title [STATUS]" with a bracketed status
...
# Resolve the "P{N}{x}: Title" milestone. Milestones are created up front,
# so this only looks one up — it never creates.
milestone=$(gh api "repos/${repo}/milestones?state=all&per_page=100" \
  --jq "[.[].title | select(startswith(\"P${ms_num}:\"))][0] // empty")
[[ -n "$milestone" ]] || {
  echo "No 'P${ms_num}: …' milestone found in ${repo}" >&2
  exit 1
}
...
while IFS=$'\t' read -r num title slug; do
  ...
  {
    printf 'Sub-scope of **[P%s plan](%s#%s)**.\n\n---\n' \

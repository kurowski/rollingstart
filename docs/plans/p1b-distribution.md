...
| An in-tree map's `.claude/settings.json` (the marketplace and `enabledPlugins`) offers to install `rolling` when a learner trusts the folder | 1b.8 | Open. On the maintainer's host, opening Claude Code in Homie after the settings landed installed nothing: the folder had been trusted long before the file existed, so no trust dialog fired. To probe in a never-trusted clone. Until then Homie's contributing page gives the two `/plugin` lines and a `/clear` explicitly. |
...
with `rolling` installed by hand (the trust-prompt row above):

...
`participantId` that flips an invite from pending to responded.
...
`getPollResults`, `getPollParticipants`, `listPolls`); the legacy vote
path in `trpc/routers/polls/participants.ts` (`add` at line 288 mints
...
(`20260905170000_require_participant_token`), purge, drop.

Task sources for the tutor: `af3d9273` (#3191) is the exemplary one,
ten lines in `participants.ts` plus the assertion in
`tests/email-invites.spec.ts`; its verifier is `integration
tests/email-invites.spec.ts`, which needs the stack. `6b30747e` (#3193)
removes the participant soft-delete filters from reads and is a good
"understand why this is now safe" task. Any task that changes a model
needs the migration in its brief and `regenerate-client` as a `setup:`
operation, so the type check sees the new client.

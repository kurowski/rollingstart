...
class KeepTaskTest(WorldTest):
    def test_kept_copy_drops_the_run_lines(self):
        ...
        # Two keeps in one second are two files, not one overwritten.
        w.held_task("b", w.fix)
        self.assertRuns("keep-task")
        w.held_task("b", w.fix, scope=["src/other"])
        self.assertRuns("keep-task")
        names = sorted(p.name for p in (w.learner.tasks / "greet-politely").iterdir())
        self.assertEqual(len(names), 3)
        self.assertTrue(any(n.endswith("-2.md") for n in names) or len({n[:15] for n in names}) == 3, names)
        self.assertRuns("close-task")
